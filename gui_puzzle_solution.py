...
class Solution_pop_up(tk.Tk):
    def __init__(self, game_board, alg_name, sqrt_size):
        super().__init__()
        ...
        if alg_name == 'Smart Brute Force':
            start = tt()
            valid, solution = brute_force(game_board, length)
            end = tt()

        elif alg_name == 'Algorithm 1':
            start = tt()
            valid, solution = solution_one(game_board)
            end = tt()
        else:
            start = tt()
            valid, solution = solution_two(game_board)
            end = tt()
        ...
        if valid == 1:
            ...
            message2 = tk.Label(solution_frame,bg = 'white', text = 'No Solution!', font = ('DejaVu Sans', 32)).pack()
