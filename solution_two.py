...
The algorithm then looks at row and column sums for each ith value array as well as a sum along the 0th axis. 
This last sum is the set of available numbers for the spot discussed in solution one.  The algorithm then chooses the 
the following pair with a minimum sum (value, row) (using the column sums), (value, column) (using row sums), 
(row,column) (using spot sums).  The algorithm then chooses the third dimension by taking the minimum sum of the 
other two values. So for example if (value,row) has a minimum column sum of two, then there are two columns to choose 
from, say column1 and column2.  The algorithm computes (value,column1)+(row,column1) and (value,column2)+(row,column2)
and chooses the column resulting in the minimum value. I decided to use the choose these minimums so as to gain 
certainty about a spots value as quickly as possible, but I do not claim this is optimal.  My thought is that
if I can choose between a spot with two possible values and a spot with nine possible values, in the first case
we will know the spots value whether it guessed right or wrong, in the latter case, if we chose wrong then 
there are still 8 possible choices.  

The program then fills in the selected guess on a copy on the board and recursively calls itself.
"""
